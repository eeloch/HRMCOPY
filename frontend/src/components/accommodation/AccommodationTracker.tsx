...
/** Rooms, beds and who lives where. Lives on the Employees page, Accommodation tab. */
  ...
  return (
    <div className="mt-6">
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          {([["rooms", "Rooms"], ["people", "People"]] as [string, string][]).map(([key, label]) => (
            <button key={key} type="button" onClick={() => setTab(key as "rooms" | "people")} className={`rounded-full px-4 py-2 text-sm font-semibold ${tab === key ? "bg-slate-900 text-white" : "bg-white text-slate-700 hover:bg-slate-50"}`}>{label}</button>
          ))}
        </div>
        ...
        {!loading && tab === "rooms" && (
          <>
            ...
            {!buildings.length && <AppCard><p className="p-8 text-center text-slate-500">No rooms recorded yet. Load them with Bulk Import on the Employees page - the staff file&apos;s Accommodation and ROOM ALLOCATED columns place everyone.</p></AppCard>}
          </>
        )}
