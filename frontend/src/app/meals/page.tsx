...
type Collection = { id: number; employee_name: string; work_date: string; timestamp: string; device: string; entitlement: number; sequence: number; rate: string; status: string; voided: boolean; void_reason: string };
...
export default function MealsPage() {
  ...
  return <div className="min-h-screen bg-slate-100"><Sidebar /><main className="ml-64 min-w-0 p-4 md:p-8">
    ...
    {loading ? <div className="h-64 animate-pulse rounded-2xl bg-slate-200" /> : !canAccess ? <Section title="Meals Access" subtitle="Your account does not have a Meals capability."><p className="p-8 text-sm text-slate-600">Ask an administrator to grant a Meals permission if you need access to operations or configuration.</p></Section> : <>
      ...
      <Section title="Operations" subtitle="Historical entitlement and rate snapshots are preserved.">{collections.length ? <div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-left"><thead className="bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3">Employee</th><th className="px-5 py-3">Work Date</th><th className="px-5 py-3">Scan Time</th><th className="px-5 py-3">Device</th><th className="px-5 py-3">Sequence</th><th className="px-5 py-3">Entitlement</th><th className="px-5 py-3">Rate</th><th className="px-5 py-3">Status</th>{canReview && <th className="px-5 py-3">Actions</th>}</tr></thead><tbody className="divide-y divide-slate-100">{collections.map((item) => <tr key={item.id} className={item.voided ? "bg-slate-50 text-slate-400 line-through" : ""}><td className="px-5 py-4 font-medium">{item.employee_name}</td><td className="px-5 py-4 text-sm text-slate-600">{day(item.work_date)}</td><td className="px-5 py-4 text-sm text-slate-600">{dateTime(item.timestamp)}</td><td className="px-5 py-4 text-sm text-slate-600">{item.device}</td><td className="px-5 py-4 text-sm text-slate-600">{item.sequence}</td><td className="px-5 py-4 text-sm text-slate-600">{item.entitlement}</td><td className="px-5 py-4 text-sm text-slate-600">{money(item.rate)}</td><td className="px-5 py-4 no-underline">{item.voided ? <span className="inline-block no-underline"><StatusBadge status="voided" /><span className="mt-1 block max-w-xs text-xs text-slate-500">{item.void_reason}</span></span> : <StatusBadge status={item.status} />}</td>{canReview && <td className="px-5 py-4">{!item.voided && <button type="button" disabled={acting} onClick={() => setVoidId(item.id)} className="rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700">Void</button>}</td>}</tr>)}</tbody></table></div> : <Empty message="No meal collections found." />}</Section>
