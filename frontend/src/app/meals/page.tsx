...
export default function MealsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setCollections(operations.collections || []); setExceptions(operations.exceptions || []); setDevices(deviceData.results || []); setRates(rateData.results || []); setEntitlements(entitlementData.results || []); setRules(ruleData.results || []);
  ...
  async function declineExcess() {
    if (declineId === null || !declineReason.trim()) return;
  ...
  async function voidTicket() {
    if (voidId === null || !voidReason.trim()) return;
  ...
  async function loadVendorData(periodId: string) {
  ...
  const excessOutcome: Record<string, string> = { approved: "Accepted - deducted from pay", deducted: "Accepted - deducted from pay", cancelled: "Waived - no deduction", declined: "Declined - not billed" };
  ...
  function ticketActions(item: Collection) {
    ...
    if (item.excess_id !== null && item.excess_status === "pending") {
      return <div className="flex flex-wrap gap-2"><button type="button" disabled={acting} onClick={() => void approve({ id: item.excess_id as number, work_date: item.work_date })} className="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-semibold text-white disabled:opacity-50">Accept</button><button type="button" disabled={acting} onClick={() => setCancelId(item.excess_id)} className="rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700">Waive</button><button type="button" disabled={acting} onClick={() => setDeclineId(item.excess_id)} className="rounded-lg border border-red-200 px-3 py-2 text-xs font-semibold text-red-700">Decline</button></div>;
    }
    return <span className="text-xs text-slate-500">{item.excess_status ? excessOutcome[item.excess_status] || item.excess_status : ""}</span>;
  }
  const counted = collections.filter((item) => !item.voided);
  const within = counted.filter((item) => item.status === "within_entitlement").length;
  const excess = counted.filter((item) => item.status !== "within_entitlement").length;
  ...
  return <div className="min-h-screen bg-slate-100"><Sidebar /><main className="ml-64 min-w-0 p-4 md:p-8">
    ...
    {loading ? <div className="h-64 animate-pulse rounded-2xl bg-slate-200" /> : !canAccess ? <Section title="Meals Access" subtitle="Your account does not have a Meals capability."><p className="p-8 text-sm text-slate-600">Ask an administrator to grant a Meals permission if you need access to operations or configuration.</p></Section> : <>
      ...
      <Section className="mt-6" title="Entitlement Rules" subtitle="Suggested entitlement rules HR can edit. Position-specific rules should use a lower priority number so they are matched before the general rule.">{canConfigure && <form onSubmit={createRule} className="grid gap-3 border-b border-slate-200 p-5 md:grid-cols-3"><select value={ruleForm.employment_category} onChange={(event) => setRuleForm({ ...ruleForm, employment_category: event.target.value })} className={inputClass}><option value="">Any category</option>{employmentCategoryOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select value={ruleForm.employment_type} onChange={(event) => setRuleForm({ ...ruleForm, employment_type: event.target.value })} className={inputClass}><option value="">Any employment type</option>{employmentTypeOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select value={ruleForm.position} onChange={(event) => setRuleForm({ ...ruleForm, position: event.target.value })} className={inputClass}><option value="">Any position</option>{positions.map((position) => <option key={position.id} value={position.id}>{position.department_name} - {position.name}</option>)}</select><input type="number" min="0" step="1" value={ruleForm.minimum_months_of_service} onChange={(event) => setRuleForm({ ...ruleForm, minimum_months_of_service: event.target.value })} placeholder="Minimum months of service" className={inputClass} /><input required type="number" min="1" step="1" value={ruleForm.tickets_per_work_day} onChange={(event) => setRuleForm({ ...ruleForm, tickets_per_work_day: event.target.value })} placeholder="Tickets per WORK day" className={inputClass} /><input required type="number" min="0" step="1" value={ruleForm.priority} onChange={(event) => setRuleForm({ ...ruleForm, priority: event.target.value })} placeholder="Priority (lower matches first)" className={inputClass} /><input value={ruleForm.description} onChange={(event) => setRuleForm({ ...ruleForm, description: event.target.value })} placeholder="Description" className={`${inputClass} md:col-span-2`} /><div className="text-right"><button disabled={acting} className="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white">Add Rule</button></div></form>}{rules.length ? <div className="overflow-x-auto"><table className="w-full min-w-[950px] text-left"><thead className="bg-slate-50 text-xs font-semibold uppercase text-slate-500"><tr><th className="px-5 py-3">Description</th><th className="px-5 py-3">Category</th><th className="px-5 py-3">Type</th><th className="px-5 py-3">Position</th><th className="px-5 py-3">Min. Months</th><th className="px-5 py-3">Tickets</th><th className="px-5 py-3">Priority</th><th className="px-5 py-3">Status</th>{canConfigure && <th className="px-5 py-3">Actions</th>}</tr></thead><tbody className="divide-y divide-slate-100">{rules.map((rule) => <tr key={rule.id}><td className="px-5 py-4 font-medium">{rule.description || "-"}</td><td className="px-5 py-4 text-sm text-slate-600">{rule.employment_category || "Any"}</td><td className="px-5 py-4 text-sm text-slate-600">{rule.employment_type || "Any"}</td><td className="px-5 py-4 text-sm text-slate-600">{rule.position_name || "Any"}</td><td className="px-5 py-4 text-sm text-slate-600">{rule.minimum_months_of_service ?? "-"}</td><td className="px-5 py-4">{rule.tickets_per_work_day}</td><td className="px-5 py-4 text-sm text-slate-600">{rule.priority}</td><td className="px-5 py-4"><StatusBadge status={rule.active ? "active" : "inactive"} /></td>{canConfigure && <td className="px-5 py-4"><button type="button" disabled={acting} onClick={() => void toggleRule(rule)} className="rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold">{rule.active ? "Deactivate" : "Activate"}</button></td>}</tr>)}</tbody></table></div> : <Empty message="No meal entitlement rules configured." />}</Section>
