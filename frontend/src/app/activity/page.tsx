...
function eventLink(event: AuditEvent): string | null {
  if (event.object_type === "attendance.attendanceexception" && event.object_id) {
    return `/attendance/exceptions?id=${event.object_id}`;
  }
    ...
    <div className="min-h-screen bg-slate-100">
      ...
      <main className="ml-64 min-w-0 p-4 md:p-8">
        <PageHeader
          ...
          description="A chronological record of completed workforce operations."
          actions={<button onClick={() => void loadActivity()} className="rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50">Refresh</button>}
        />
        {error ? <ErrorPanel message={error} retry={loadActivity} /> : (
          <>
            <AppCard className="mb-6" title="Filters">
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <input value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => event.key === "Enter" && void loadActivity(true)} placeholder="Search activity..." className="rounded-xl border border-slate-300 px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500 xl:col-span-2" />
                <input value={employee} onChange={(event) => setEmployee(event.target.value)} onKeyDown={(event) => event.key === "Enter" && void loadActivity(true)} placeholder="Employee ID" className="rounded-xl border border-slate-300 px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500" />
                <select value={module} onChange={(event) => setModule(event.target.value)} className="rounded-xl border border-slate-300 bg-white px-4 py-3"><option value="">All modules</option>{modules.map((item) => <option key={item} value={item}>{capitalize(item)}</option>)}</select>
                <select value={severity} onChange={(event) => setSeverity(event.target.value)} className="rounded-xl border border-slate-300 bg-white px-4 py-3"><option value="">All severities</option>{severities.map((item) => <option key={item} value={item}>{capitalize(item)}</option>)}</select>
                <input type="date" value={dateFrom} onChange={(event) => setDateFrom(event.target.value)} className="rounded-xl border border-slate-300 px-4 py-3" />
                <input type="date" value={dateTo} onChange={(event) => setDateTo(event.target.value)} className="rounded-xl border border-slate-300 px-4 py-3" />
                <div className="flex gap-2">
                  <button onClick={() => void loadActivity(true)} className="rounded-xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white hover:bg-blue-700">Apply</button>
                  <button onClick={clearFilters} className="rounded-xl border border-slate-300 px-4 py-3 text-sm font-medium text-slate-700 hover:bg-slate-50">Clear</button>
                </div>
              </div>
            </AppCard>
                  ...
                  {events.map((event) => {
                    ...
                    return (
                      <article
                        ...
                        className={`flex gap-4 px-5 py-5 ${link ? "cursor-pointer transition hover:bg-blue-50/50" : ""}`}
                      >
                        <div className={`mt-1 h-3 w-3 shrink-0 rounded-full ${dotColor(event.severity)}`} />
                        <div className="min-w-0 flex-1">
                          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                            ...
                            <div className="flex shrink-0 items-center gap-2">
                              ...
                              {link && <span className="text-xs font-semibold text-blue-600">View →</span>}
                            </div>
                          </div>
                          <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-sm text-slate-500">
                            <span>{formatDateTime(event.created_at)}</span>
                            <span>{capitalize(event.module)}</span>
                            {event.actor_name && <span>By {event.actor_name}</span>}
                            {event.employee_id && (
                              <button
                                onClick={(domEvent) => { domEvent.stopPropagation(); router.push(`/employees/${event.employee_id}`); }}
                                className="font-medium text-blue-700 hover:underline"
                              >
                                {event.employee_name || event.employee_number || "View employee"}
                              </button>
                            )}
                            {event.employee_department && <span>{event.employee_department}</span>}
                          </div>
                        </div>
                      </article>
                    );
